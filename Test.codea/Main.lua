...
function draw()
    ...
    if flapper.body.x + WIDTH*0.75 + 100 > next then
        spawnNext()
    end
    ...
    local seg = 100
    local offset = -camera
    for i = offset, offset + WIDTH + seg * 2, seg do
    ...
    if state == MENU then
        ...
        font("Arial-BoldMT")
        fontSize(40)
        text("Touch to flap", WIDTH * 0.25, HEIGHT * 0.4)
    ...
    if readLocalData("score") then
        ...
        textAlign(LEFT)
        fontSize(20)
        text("HIGH "..readLocalData("score"), 75, HEIGHT - 30)
    end
...
function touched(touch)
    if touch.state == BEGAN then
        ...
        elseif state == GAMEOVER and gameOverTimer <= 0 then
            print("Dead")
            alert("You fucked Up,Stupid Ass")
